...
export class FileExceptionFilter implements ExceptionFilter {
  ...
  private extractFileType(message: string) {
    const mediaTypeStr = extractFromText(message, this.FILE_TYPE_REGEX);
    const mediaTypeWithBackslashes = mediaTypeStr.split('|');
    const mediaTypes = mediaTypeWithBackslashes.map((type) =>
      type.replace('\\', ''),
    );
  ...
  private readonly FILE_TYPE_REGEX = /\/(.*)\//;
  ...
  private createErrorData(message: string) {
    ...
    if (message.includes(this.MessageSnippet.MAX_SIZE)) {
      ...
      expectedFileTypes = this.extractFileType(message);
